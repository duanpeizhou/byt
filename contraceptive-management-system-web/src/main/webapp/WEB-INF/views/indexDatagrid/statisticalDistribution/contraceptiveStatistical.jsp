...
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:f="http://java.sun.com/jsf/core" xmlns:h="http://java.sun.com/jsf/html">
	<head>
		...
		<script type="text/javascript">
		 $.extend($.fn.combobox.defaults, {panelHeight:true});
			...
			$(function(){
				$.post('contraceptiveStatisticalDatagrid',function(json){
					var root=json.rows; 
					var arr=new Array();
			...
			function columnChart(arr){
            	var myChart = new FusionCharts( "FusionCharts/Column3D.swf", "myChartId", "550", "300", "0" );
		           ...
		           "data" : 
		           [ 
						{ "label" : "避孕套（大）", "value" : arr[0] },
						{ "label" : "避孕套（中）", "value" : arr[1] }, 
						{ "label" : "避孕栓", "value" : arr[2] },
						{ "label" : "避孕套（小）", "value" : arr[3] }
		           ]
  			 });
   		    myChart.render("chartContainer");
            }
			/* function columnChart(root){
            	var myChart = new FusionCharts( "FusionCharts/Column3D.swf", "myChartId", "550", "300", "0" );
            	var ss='{ "chart": { "caption" : "" , ' +
   		        '"xAxisName" : "", "yAxisName" : "总共个数","formatnumberscale":"0", "numberSuffix" : "个" }, ' +
   		        '"data" : [ ';
				for(var i=0;i<root.length;i++){
            		ss+='{ "label" : "'+root[i].contraceptive.name+'", "value" : "'+root[i].total+'" },';
				}
				ss=ss.substring(0, ss.length-1);
				ss+=']} ';
       			myChart.setJSONData(ss);
   		    myChart.render("chartContainer");
            } */
			...
			function submitForm(){
				...
				$('#dg').datagrid({
					url : 'contraceptiveStatisticalDatagrid',
					queryParams : params
				});
			}
			function nameFomatter(value,row,index){
	...
	<body class="easyui-layout">
		    ...
		    <div data-options="region:'east',title:'数据统计',split:true" style="width:600px;">
		    ...
		    <div data-options="region:'center',title:'数据列表'">
				...
				data-options="onLoadSuccess:selectOneLine,striped:true,rownumbers:true,fit:true,loadMsg:'加载中，请稍后...',pageList:[10,25,40,50],pagination:true,url:'contraceptiveStatisticalDatagrid',singleSelect:true,pageSize:25">
				<thead frozen="true">
					<tr>
						<th field="contraceptive.name" data-options="formatter:nameFomatter,fixed:true,width:80" align="center">药具品种</th>
						</tr>
				</thead>
				<thead>
				<tr>
