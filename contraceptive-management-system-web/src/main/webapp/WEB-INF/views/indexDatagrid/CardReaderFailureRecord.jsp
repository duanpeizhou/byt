	...
	<head>
	 ...
	 function submitForm(){
    		...
    		params.endTime = $("#endTime").datetimebox("getValue").length==0?-1:$("#endTime").datetimebox("getValue");
          	...
          	$('#dg').datagrid({
          		url: 'getCardReaderFailureRecordSearchArea',
          		queryParams:params
          	});
          }
		...
		<div data-options="region:'north',title:'条件查询',split:true" style="height:80px;text-align:left;padding:10px;" align="center">
			 <form id="form" method="post">  
			   ...
			   <jsp:include page="../base/areaSearch.jsp" />
			    故障时间自：<input id="startTime" class="easyui-datetimebox" name="birthday" data-options=""  style="width:90px"/>
		 ...
		 <table id="dg" class="easyui-datagrid" style="width: 570px;height: 250px;"
				data-options="striped:true,fit:true,loadMsg:'加载中，请稍后...',pageList:[5,10,15],pagination:true,onLoadSuccess:selectOneLine,url:'getCardReaderFailureRecordSearchArea',singleSelect:true,pageSize:10">
			<thead frozen="true">
				<tr>
			...
			<thead>
				<tr>
					...
					<th field="failureDate" data-options="sortable:true,fixed:true,width:160,align:'center'">故障时间</th>
					<th field=recoveryDate data-options="sortable:true,fixed:true,width:160,align:'center'">恢复时间</th>
				</tr>
			</thead>
		</table>
	</div>
