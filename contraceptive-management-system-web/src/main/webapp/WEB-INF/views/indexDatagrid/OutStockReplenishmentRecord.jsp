	...
	<head>
	    ...
	    <script src="js/commonJS1.js" type="text/javascript"></script>
	      ...
	      $.extend($.fn.combobox.defaults, {panelHeight:true});
            ...
            function aisleNumFormatter(value,row,index){
			 ...
			 function submitForm(){
	            	...
	            	$('#dg').datagrid({
	            		url: 'getOutStockReplenishmentRecordsDatagrid',
	            		queryParams:params
	            	});
         	   }
			 function downloadData(){
					...
					endTime = $("#endTime").datetimebox("getValue").length==0?-1:$("#endTime").datetimebox("getValue");
			     	communityId = $("#community").combobox('getValue') != "全部"
							&& $("#community").combobox('getValue') > 0 ? $(
							"#community").combobox('getValue') : -1;
						$("<iframe src='getOutStockReplenishmentRecordsDatagrid/export?cityId="+'<spring:message code="City.no"/>'+"&startTime="+startTime+"&endTime="+endTime
		...
		<div data-options="region:'north',title:'条件查询',split:true" style="height:80px;text-align:left;padding:10px;" align="center">
			 <form id="form" method="post">  
			   ...
			   <jsp:include page="../base/areaSearch.jsp" />
			    缺货时间自：<input id="startTime" class="easyui-datetimebox" name="birthday" data-options=""  style="width:90px"/>
		 ...
		 <table id="dg" class="easyui-datagrid" style="width: 570px;height: 250px;"
				data-options="onLoadSuccess:selectOneLine,striped:true,fit:true,loadMsg:'加载中，请稍后...',pageList:[10,25,50],pagination:true,url:'getOutStockReplenishmentRecordsDatagrid',singleSelect:true,pageSize:10">
			<thead frozen="true">
				<tr>
			...
			<thead>
				<tr>
					...
					<th field=machineryEquipment.terminalType data-options="formatter:deviceTypeFormatter,sortable:true,fixed:true,width:80" align="center">终端类型</th>
					...
					<th field="machineryEquipment.area.parentArea.parentArea.name" data-options="formatter:countryArea,fixed:true,width:80" align="center">区（县）</th>
					<th field="machineryEquipment.area.parentArea.name" data-options="formatter:townStreetArea,fixed:true,width:120" align="center">街道（乡镇）</th>
					<th field="machineryEquipment.area.name" data-options="formatter:communityArea,fixed:true,width:100" align="center">社区</th>
					...
					<th field=contraceptive.name data-options="formatter:aisleNumFormatter" align="center">缺货名称 </th>
					<th field="outStockDate" align="center" data-options="sortable:true,fixed:true,width:160">缺货时间</th>
					<th field="replenishmentDate" align="center" data-options="sortable:true,fixed:true,width:160">补货时间</th>
				</tr>
			</thead>
		</table>
	</div>
