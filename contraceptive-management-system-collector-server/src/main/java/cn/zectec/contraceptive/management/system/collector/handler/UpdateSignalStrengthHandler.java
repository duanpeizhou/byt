...
public class UpdateSignalStrengthHandler implements MessageReceivedHandler{
	@Autowired
	...
	public void handle(Context context, Request request) {
		if(request.getMessage() instanceof StatusReportRequestMessage){
			...
			machineryEquipmentService.updateSignalStrength(me.getId(),((StatusReportRequestMessage)request.getMessage()).getSignalStrength());
		}
