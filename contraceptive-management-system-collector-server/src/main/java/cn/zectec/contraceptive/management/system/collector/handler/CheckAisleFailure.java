...
public class CheckAisleFailure implements MessageReceivedHandler{
	@Autowired
	...
	public void handle(Context context, Request request) {
		if(request.getMessage() instanceof StatusReportRequestMessage){
			...
			machineryEquipmentService.checkAisleFailure(me.getId(), ((StatusReportRequestMessage)request.getMessage()).getAilseFailure());
		}
